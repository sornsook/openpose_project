...
class TfPoseEstimator:
    ...
    def draw_humans(npimg, humans, imgcopy=False):
        ...
        return npimg
    #######################################################################################
    def get_part_location(npimg, humans, imgcopy=False):
        # loc_x = np.ones(19)*(-1)
        # loc_y = np.ones(19)*(-1)
        loc   = np.zeros([2,19])+1*(-1)
        image_h, image_w = npimg.shape[:2]
        # for human in humans:
        if len(humans)>0:
            human = humans[0] # get only one person
            ...
            for i in range(common.CocoPart.Background.value):
                ...
                body_part   = human.body_parts[i]
                loc[0,i]    = int(body_part.x * image_w + 0.5)
                loc[1,i]    = int(body_part.y * image_h + 0.5)
